...
## 2. Подготовь staging
- Не коммить `src/styles/variables.css` если в diff только whitespace (отступы 2→3 пробела) — реверти этот файл
- Не коммить `.claude/settings.local.json` (локальные настройки)
- Добавь только релевантные файлы — никаких `git add -A` или `git add .`
